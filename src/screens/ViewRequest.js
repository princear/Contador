import {
  ...
  Modal, TextInput, ScrollView, ImageBackground, Image, Button, Linking
} from 'react-native'
...
const ViewRequest = ({ route }) => {
  ...
  const gotoViewFile = (myfile, type) => {
    ...
    // if (type == 'pdf') {
    //   Linking.openURL(`data:image/jpeg;base64,${myfile}`)
      ...
      >
        ...
        >
          <View style={styles.container}>
            ...
            {REQUEST_INFO_BY_ID?.actionModel?.fileType ?
              <View style={{ backgroundColor: '#fff', paddingBottom: 20, width: wp(90), alignSelf: 'center', marginTop: 20 }}>
                <View style={[styles.slideContainerClient1, { backgroundColor: '#254768' }]}>
                  <View style={{ marginLeft: 15, }}>
                    <Text style={{ fontSize: 14, color: '#fff', fontFamily: 'Poppins-SemiBold', }}>Attachments</Text>

                  </View>
                </View>
                <View style={{ marginLeft: 15, marginBottom: 10, marginTop: 10 }}>
                  <Text style={{ fontSize: 12, fontFamily: 'Poppins-SemiBold', color: Color.headerIconBG, }}>Type: <Text style={{ fontSize: 12, color: Color.darkGreen }}>{REQUEST_INFO_BY_ID?.actionModel?.fileType}</Text> </Text>

                  {REQUEST_INFO_BY_ID?.actionModel?.fileType == 'pdf' ?
                    <TouchableOpacity
                      onPress={() => gotoViewFile(REQUEST_INFO_BY_ID?.actionModel?.fileData, REQUEST_INFO_BY_ID?.actionModel?.fileType)}

                    >
                      <Pdf
                        source={{ uri: `data:application/pdf;base64,${REQUEST_INFO_BY_ID?.actionModel?.fileData}` }}
                    ...
                    >
                      <Image
                        source={{ uri: `data:image/jpeg;base64,${REQUEST_INFO_BY_ID?.actionModel?.fileData}` }}
